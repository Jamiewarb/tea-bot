...
module.exports = function(controller) {
    controller.hears(['^random'], 'direct_mention,mention', function(bot, message) {
        confirmRandom(controller, bot, message);
    });

    controller.on('interactive_message_callback', function(bot, trigger) {

        if (trigger.actions[0].name.match(/^start_random$/)) {
            confirmRandom(controller, bot, {
                'team': trigger.team,
                'user': trigger.user,
        ...
        if (trigger.actions[0].name.match(/^confirm_random$/)) {
...
const respondRandom = function(bot, trigger, response, outcome) {
    // if (outcome === 'start') {
...
const confirmRandom = function(controller, bot, message) {
